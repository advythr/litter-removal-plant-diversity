...
```{r}
qqnorm(analysis.df$LIT)
qqline(analysis.df$LIT)
```
...
analysis.df2 <- analysis.df %>%
  select(-BG) %>% # Since BG is 1-LIT, we don't need it anymore for the analysis)
...
qqnorm(analysis.df$LIT) # the original one
qqline(analysis.df$LIT)

qqnorm(analysis.df2$log_LIT)
qqline(analysis.df2$log_LIT) # slightly better!
```
...
```{r}
ggsave(
  "analyses_manuscript/figures/litter_cover_by_treatment.pdf",
  plot = p_lit,
  device = "pdf",
  height = 6,
  width = 10,
...
```{r}
anova.lit <- aov(LIT ~ year_treatment, analysis.df)
summary(anova.lit)
TukeyHSD(anova.lit)
```
...
```{r}
check_my_model(litter.lmm, dataset = analysis.df)
```
